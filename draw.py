...
class QuotaDrawer:
    ...
    def _draw_usage_section(self, draw: ImageDraw.ImageDraw, x: int, label_y: int, width: int,
                           label: str, percent: float, bar_y: int) -> int:
        draw.text((x + self.CARD_PADDING_X, label_y), label, font=self.font_label, fill=self.COLOR_TEXT_LABEL)

        percent_text = f"{percent:.1f}%"
        bbox_percent = draw.textbbox((0, 0), percent_text, font=self.font_usage)
        percent_w = bbox_percent[2] - bbox_percent[0]
        draw.text((x + width - self.CARD_PADDING_X - percent_w, bar_y - self.CARD_BAR_Y_OFFSET + self.CARD_PERCENT_Y_OFFSET),
                 percent_text, font=self.font_usage, fill=self.COLOR_TEXT_USAGE)

        bar_x = x + self.CARD_PADDING_X
        bar_w = width - self.CARD_PADDING_X * 2

        self._draw_rounded_rect(draw, (bar_x, bar_y, bar_x + bar_w, bar_y + self.BAR_HEIGHT),
                               self.BAR_RADIUS, fill=self.COLOR_PROGRESS_BG)
        fill_w = int(bar_w * min(percent / 100, 1.0))
        if fill_w > 0:
            self._draw_rounded_rect(draw, (bar_x, bar_y, bar_x + fill_w, bar_y + self.BAR_HEIGHT),
                                   self.BAR_RADIUS, fill=self._get_progress_color(percent))

        return bar_y + self.BAR_HEIGHT
    ...
    def _draw_card(self, draw: ImageDraw.ImageDraw, x: int, y: int, width: int, model_name: str,
        ...
        card_h = self.CARD_HEIGHT_WITH_WEEK if has_week_limit else self.CARD_HEIGHT_WITHOUT_WEEK
        ...
        draw.text((x + self.CARD_PADDING_X, y + self.CARD_MODEL_NAME_Y), model_name,
                 font=self.font_model, fill=self.COLOR_TEXT_MODEL)

        intv_remain = intv_total - intv_used
        intv_percent = (intv_remain / intv_total * 100) if intv_total > 0 else 0

        if intv_total == 0:
            draw.text((x + self.CARD_PADDING_X, y + self.CARD_LABEL_START_Y),
                     f"{intv_label}：无限额", font=self.font_label, fill=self.COLOR_TEXT_LABEL)
        else:
            intv_bar_y = y + self.CARD_LABEL_START_Y + self.CARD_BAR_Y_OFFSET
            self._draw_usage_section(draw, x, y + self.CARD_LABEL_START_Y, width,
                                    intv_label, intv_percent, intv_bar_y)

        if has_week_limit:
            intv_end_y = y + self.CARD_LABEL_START_Y + self.CARD_BAR_Y_OFFSET + self.BAR_HEIGHT
            week_start_y = intv_end_y + self.CARD_WEEK_START_OFFSET
            week_bar_y = week_start_y + self.CARD_BAR_Y_OFFSET
            week_remain = week_total - week_used
            week_percent = (week_remain / week_total * 100) if week_total > 0 else 0

            week_percent_y = intv_end_y + self.CARD_WEEK_PERCENT_OFFSET
            draw.text((x + self.CARD_PADDING_X, week_start_y), "周使用/总额",
                     font=self.font_label, fill=self.COLOR_TEXT_LABEL)
            percent_text = f"{week_percent:.1f}%"
            bbox_percent = draw.textbbox((0, 0), percent_text, font=self.font_usage)
            percent_w = bbox_percent[2] - bbox_percent[0]
            draw.text((x + width - self.CARD_PADDING_X - percent_w, week_percent_y),
                     percent_text, font=self.font_usage, fill=self.COLOR_TEXT_USAGE)

            bar_x = x + self.CARD_PADDING_X
            bar_w = width - self.CARD_PADDING_X * 2

            self._draw_rounded_rect(draw, (bar_x, week_bar_y, bar_x + bar_w, week_bar_y + self.BAR_HEIGHT),
                                   self.BAR_RADIUS, fill=self.COLOR_PROGRESS_BG)
            fill_w = int(bar_w * min(week_percent / 100, 1.0))
            if fill_w > 0:
                self._draw_rounded_rect(draw, (bar_x, week_bar_y, bar_x + fill_w, week_bar_y + self.BAR_HEIGHT),
                                       self.BAR_RADIUS, fill=self._get_progress_color(week_percent))

    def _calculate_height(self, model_cards: List[Dict]) -> int:
        card_heights = [self.CARD_HEIGHT_WITH_WEEK if c["has_week_limit"] else self.CARD_HEIGHT_WITHOUT_WEEK
                       for c in model_cards]
        total_cards_h = sum(card_heights) + len(card_heights) * self.CARD_SPACING if card_heights else 0
        return (self.PADDING + self.HEADER_HEIGHT + self.SECTION_SPACING +
                total_cards_h + self.SECTION_SPACING +
                self.INFO_HEIGHT + self.SECTION_SPACING +
    ...
    def _draw_all(self, draw: ImageDraw.ImageDraw, plan_name: str, model_cards: List[Dict],
        ...
        self._draw_rounded_rect(draw, (0, y_offset, self.IMG_WIDTH, y_offset + self.HEADER_HEIGHT),
        ...
        y_offset += self.HEADER_HEIGHT + self.SECTION_SPACING
        ...
        for card in model_cards:
            self._draw_card(draw, self.PADDING, y_offset, self.CARD_WIDTH,
                          ...
                          card["has_week_limit"])
            card_h = self.CARD_HEIGHT_WITH_WEEK if card["has_week_limit"] else self.CARD_HEIGHT_WITHOUT_WEEK
            y_offset += card_h + self.CARD_SPACING

        self._draw_rounded_rect(draw, (self.PADDING, y_offset, self.IMG_WIDTH - self.PADDING, y_offset + self.INFO_HEIGHT),
                               radius=12, fill=self.COLOR_HEADER_BG)
